...
function cm.filter2(c,e,tp,m,f,chkf)
	return c:IsType(TYPE_FUSION) and (not f or f(c)) and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_FUSION,tp,false,false,POS_FACEDOWN_DEFENSE) and c:CheckFusionMaterial(m,nil,chkf) and c:IsCode(11428)
end
...
function cm.filter5(c,e,tp,m,f,chkf)
	return (c:IsType(TYPE_FUSION) or c.IsFusionSpellTrap) and (not f or f(c)) and (c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_FUSION,tp,false,false,POS_FACEDOWN_DEFENSE) or c:IsSSetable()) and c:CheckFusionMaterial(m,nil,chkf) and c:IsCode(11428)
end
...
function cm.fstg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		--mg1:Merge(mg2)
		if e:GetHandler():GetColumnGroupCount()==0 and 0==1 then
			local mg3=Duel.GetMatchingGroup(cm.filter6,tp,LOCATION_GRAVE,LOCATION_GRAVE,nil)
			mg1:Merge(mg3)
		end
		local res=Duel.IsExistingMatchingCard(cm.filter5,tp,LOCATION_DECK+LOCATION_GRAVE,0,1,nil,e,tp,mg1,nil,chkf)
		if not res then
...
function cm.fsop(e,tp,eg,ep,ev,re,r,rp)
	...
	local sg1=Duel.GetMatchingGroup(cm.filter5,tp,LOCATION_DECK+LOCATION_GRAVE,0,nil,e,tp,mg1,nil,chkf)
	local mg3=nil
	local sg2=nil
	local ce=Duel.GetChainMaterial(tp)
	if ce~=nil then
