...
function cm.spfilter2(c,e,tp,m,f,chkf)
	if not (c:IsType(TYPE_FUSION) and aux.IsMaterialListCode(c,17000) and (not f or f(c))
...
function cm.fcheck(tp,sg,fc)
	return sg:IsExists(Card.IsFusionCode,1,nil,17000)
end
function cm.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		 and Duel.IsPlayerCanSpecialSummonMonster(tp,17008,0,0x4011,0,0,4,RACE_SPELLCASTER,ATTRIBUTE_LIGHT,POS_FACEUP) and Duel.IsExistingTarget(cm.thfilter1,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil) end
...
function cm.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 or not Duel.IsPlayerCanSpecialSummonMonster(tp,17008,0,0x4011,0,0,4,RACE_SPELLCASTER,ATTRIBUTE_LIGHT,POS_FACEUP) then return end
	local token=Duel.CreateToken(tp,17008)
	if Duel.SpecialSummon(token,0,tp,tp,false,false,POS_FACEUP)~=0 then
		...
		if res and Duel.SelectYesNo(tp,aux.Stringid(m,2)) then
			...
			if ce~=nil then
				...
				mg3=fgroup(ce,e,tp)
				local mf=ce:GetValue()
				sg2=Duel.GetMatchingGroup(cm.spfilter2,tp,LOCATION_EXTRA,0,nil,e,tp,mg3,mf,chkf)
			end
			if sg1:GetCount()>0 or (sg2~=nil and sg2:GetCount()>0) then
				...
				aux.FCheckAdditional=tc.starlight_fusion_check or cm.fcheck
...
function cm.mfilter(c)
	return c:IsCode(17000) and c:IsType(TYPE_MONSTER) and c:IsType(TYPE_TUNER) 
end
...
function cm.scop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SpecialSummon(token,0,tp,tp,false,false,POS_FACEUP)~=0 then
		...
		e2:SetOperation(f)
		token:RegisterEffect(e2)
		local mg=Duel.GetMatchingGroup(Card.IsCode,tp,LOCATION_MZONE,0,nil,17000)
