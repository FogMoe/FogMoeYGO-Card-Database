...
function cm.slevel(e,c)
	...
	if c:IsOriginalCodeRule(15187) then
		return 4
	else return lv end
end
...
function cm.SynOperation(f1,f2,minct,maxc)
	return  function(e,tp,eg,ep,ev,re,r,rp,c,smat,mg,min,max)
				...
				g:DeleteGroup()
				c:RegisterFlagEffect(15187,RESET_EVENT+RESETS_STANDARD-RESET_TOFIELD,0,1)
			end
end
function cm.lvcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsSummonType(SUMMON_TYPE_SYNCHRO) and e:GetHandler():GetFlagEffect(15187)~=0
end
function cm.ttlv(c)
