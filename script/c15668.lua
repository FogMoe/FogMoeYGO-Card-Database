...
function cm.checkop(e,tp,eg,ep,ev,re,r,rp)
	...
	while tc do 
		if tc:IsReason(REASON_COST) and tc:GetReasonEffect():IsActivated() and ((tc:GetReasonEffect():IsActiveType(TYPE_MONSTER) and tc:GetReasonEffect():GetHandler():IsAttribute(ATTRIBUTE_WATER)) or tc:GetReasonEffect():GetHandler():IsCode(15661)) then
...
function cm.regcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsReason(REASON_COST) and re:IsActivated() and ((re:IsActiveType(TYPE_MONSTER) and re:GetHandler():IsAttribute(ATTRIBUTE_WATER)) or re:GetHandler():IsCode(15661))
end
