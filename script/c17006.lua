...
function cm.initial_effect(c)
    ...
    --fusion material
    c:EnableReviveLimit()
    aux.AddFusionProcCodeFun(c,66917000,aux.FilterBoolFunction(Card.IsAttribute,ATTRIBUTE_EARTH),1,true,true) 
    --cannot target
...
function cm.starlight_fusion_check(tp,sg,fc)
    return aux.gffcheck(sg,Card.IsFusionCode,66917000,Card.IsAttribute,ATTRIBUTE_EARTH)
end
function cm.tgcon(e)
